...
/// <summary>
/// Level-2 fan-out for <c>MissionDeactivatedIntegrationEvent</c>: consumed independently by
/// SessionService and StageService, with the same self-heal asymmetry as
/// <see cref="MissionActivatedFanOutFixture"/> (mirrored: SessionService self-heals to
/// "Inactive", StageService no-ops if absent — see <see cref="MissionDeactivatedFanOutTests"/>).
/// MissionService itself is never started, only the wire shape of the event it publishes
/// (<c>ChangeMissionStatusCommandHandler</c>, else branch) matters.
/// </summary>
public class MissionDeactivatedFanOutFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _sessionPostgres = new PostgreSqlBuilder("postgres:15-alpine")
            .WithDatabase("sessionservice_l2_missiondeactivated_it")
            .WithUsername("umbral")
            .WithPassword("umbral")
            .Build();
        _stagePostgres = new PostgreSqlBuilder("postgres:15-alpine")
            .WithDatabase("stageservice_l2_missiondeactivated_it")
            .WithUsername("umbral")
            .WithPassword("umbral")
            .Build();
    ...
    /// <summary>Publishes a <see cref="MissionDeactivatedIntegrationEvent"/> exactly as MissionService would.</summary>
    public Task PublishMissionDeactivatedAsync(Guid missionId, string name)
        => _publisherBus.Publish(new MissionDeactivatedIntegrationEvent(missionId, name, DateTime.UtcNow));
...
public class MissionDeactivatedFanOutCollection : ICollectionFixture<MissionDeactivatedFanOutFixture>
{
    public const string Name = "MissionDeactivated fan-out integration tests (Level 2 - real RabbitMQ)";
}
