...
/// rebuild path (upsert-by-id, drop-stale) runs against real Postgres.
/// </summary>
[Collection(ClueServiceCollection.Name)]
public class ClueServiceReprojectTests(ClueServicePostgresFixture fixture) : IAsyncLifetime
{
    ...
    [Fact]
    public async Task Reproject_RebuildsStagesLookup_FromUpstreamStagesFeed()
    {
        var missionId = Guid.NewGuid();
        var upstreamStages = new[]
        {
            new UpstreamStageFeedItem(Guid.NewGuid(), missionId, "Stage A"),
            new UpstreamStageFeedItem(Guid.NewGuid(), missionId, "Stage B"),
        };
        ...
        var result = JsonSerializer.Deserialize<ClueReprojectResultDto>(
        ...
        var rowCount = await dbContext.StagesLookup.CountAsync(s => s.MissionId == missionId);
        rowCount.Should().Be(upstreamStages.Length);
    }
}
