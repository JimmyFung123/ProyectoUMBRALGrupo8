...
public class MissionCreatedConsumerTests
{
    ...
    [Fact]
    public async Task Consume_WhenNotSeededYet_AddsLookupAndSaves()
    {
    ...
    [Fact]
    public async Task Consume_WhenAlreadySeeded_IsIdempotentAndSkips()
    {
        var missionId = Guid.NewGuid();
        _repoMock.Setup(r => r.GetByIdAsync(missionId, It.IsAny<CancellationToken>()))
